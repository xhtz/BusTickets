...
public class RegisterBajajActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Sajili Bajaj");
        setContentView(R.layout.activity_register_bajaj);
    ...
    public void registerBajaj(View view){
        ...
        if(regNumber.length()>0 && phoneNumber.length()>0 && parkingArea.length()>0 && driverName.length()>0 && streetName.length()>0)
        {
            if(phoneNumber.length()==12 || phoneNumber.length()==10)
            {
                ...
                new RegisterDriver(RegisterBajajActivity.this).execute(params);
            }
            else
            {
    ...
    void dialogMessage(String msg){
        AlertDialog.Builder builder = new AlertDialog.Builder(RegisterBajajActivity.this);
